...
plots_wd = "/projects_eg/projects/marta/TestisRestricted_Microproteins_TSA/with_TranscriptomeReconstruction/Q1_TestisORFs/human/plots"
save_wd = "/projects_eg/projects/marta/TestisRestricted_Microproteins_TSA/with_TranscriptomeReconstruction/Q1_TestisORFs/human"
```
...
print(num_ORFs_per_transcript %>% group_by(gene_type) %>% summarise(mean_n_ORFs = mean(n_ORFs), sd_n_ORFs = sd(n_ORFs)))
...
ggplot(ribORF_humanTestis_in1, aes(x=gene_type, y=length, fill=gene_type)) +
  geom_boxplot() +
  scale_fill_manual(values=c("protein_coding"="#849DB1",
                         "lncRNA" = "#B66353",
                         "processed_pseudogene" = "#FBB04E",
...
print(ribORF_humanTestis_in1 %>% group_by(gene_type) %>% summarise(median_length_aa = median(length_aa), mean_length_aa = mean(length_aa), sd_length_aa = sd(length_aa)))
...
orf_TE_in1 = orf_TE %>% 
  ...
  unique() %>% 
  subset(orfID %in% ribORF_humanTestis_in1$orfID) %>% 
  unique()
nrow(orf_TE_in1)
...
#----------- sample 1 ---------#
## riboseq
orf_testis_1 = orf_TE %>% subset(Sample == "testis_1") %>% subset(orfID %in% orf_TE_in1$orfID) %>% select(orfID, ORF_type, transcript_id, f1Tpm)
...
#----------- sample 2 ---------#
## riboseq
orf_testis_2 = orf_TE %>% subset(Sample == "testis_2") %>% subset(orfID %in% orf_TE_in1$orfID) %>% select(orfID, ORF_type, transcript_id, f1Tpm)
...
#----------- sample 3 ---------#
## riboseq
orf_testis_3 = orf_TE %>% subset(Sample == "testis_3") %>% subset(orfID %in% orf_TE_in1$orfID) %>% select(orfID, ORF_type, transcript_id, f1Tpm)
...
# orf_TE_2 = read.csv("/users/genomics/marta/TestisProject_SaraRazquin/JC_RiboSeq_TPMs/Outputs/with_TranscriptomeReconstruction/Expression/ribORFsimp.brain_2.tsv", sep="\t")
...
# #----------- sample 1 ---------#
# ## riboseq
# orf_brain_1 = orf_TE %>% subset(Sample == "testis_1") %>% subset(orfID %in% orf_TE_in1$orfID) %>% select(orfID, ORF_type, transcript_id, f1Tpm)
...
# 
# ## TE
# brain_1$TE = testis_1$Ribo_TPM / testis_1$RNA_TPM
# brain_1$sample = "Brain_1"
# 
# #----------- sample 2 ---------#
# ## riboseq
# orf_brain_2 = orf_TE %>% subset(Sample == "brain_2") %>% subset(orfID %in% orf_TE_in1$orfID) %>% select(orfID, ORF_type, transcript_id, f1Tpm)
# names(orf_brain_2)[4] = "Ribo_TPM"
# orf_brain_2 = merge(orf_brain_2, annot, by="transcript_id")
# 
# ## rnaseq
...
#   theme_classic()
# 
# ggsave(file.path(plots_wd,"PNG/translation_efficiency_persample_brain.png"), height=4.72, width=6.02)
# ggsave(file.path(plots_wd,"PDF/translation_efficiency_persample_brain.pdf"), height=4.72, width=6.02)
# 
# ## if an ORF is translated in more than 1 sample, compute the mean
# mean_brain_TE = brain_TE %>%
#   group_by(orfID, gene_type, gene_id, gene_name, ORF_type) %>%
#   summarize(mean_TE = ifelse(n() > 1, mean(TE), TE))
...
#   theme(legend.position="none")
# 
# ggsave(file.path(plots_wd,"PNG/translation_efficiency_meanTE_brain.log10.png"), height=4.72, width=5.02)
# ggsave(file.path(plots_wd,"PDF/translation_efficiency_meanTE_brain.log10.pdf"), height=4.72, width=5.02)
# 
...
#   theme(legend.position="none")
# 
# ggsave(file.path(plots_wd,"PNG/translation_efficiency_meanTE_brain.png"), height=4.72, width=5.02)
# ggsave(file.path(plots_wd,"PDF/translation_efficiency_meanTE_brain.pdf"), height=4.72, width=5.02)
```

```{r mean_TE_liver, echo=F}
...
# liver_TPMs_with_ORFs = liver_TPMs %>% subset(transcript_id %in% orf_TE_in1$transcript_id)
# 
# #----------- sample 1 ---------#
# ## riboseq
# orf_liver_1 = orf_TE %>% subset(Sample == "testis_1") %>% subset(orfID %in% orf_TE_in1$orfID) %>% select(orfID, ORF_type, transcript_id, f1Tpm)
# names(orf_liver_1)[4] = "Ribo_TPM"
# orf_liver_1 = merge(orf_liver_1, annot, by="transcript_id")
# 
# ## rnaseq
# rna_liver_1 = testis_TPMs_with_ORFs %>% select(transcript_id, human_liver_rna_1)
# names(rna_liver_1)[2] = "RNA_TPM"
# 
# liver_1 = merge(rna_liver_1, orf_liver_1, by=c("transcript_id"))
# 
# ## TE
# liver_1$TE = testis_1$Ribo_TPM / testis_1$RNA_TPM
# liver_1$sample = "Liver_1"
# 
# #----------- sample 2 ---------#
# ## riboseq
# orf_liver_2 = orf_TE %>% subset(Sample == "liver_2") %>% subset(orfID %in% orf_TE_in1$orfID) %>% select(orfID, ORF_type, transcript_id, f1Tpm)
...
# #----------- sample 3 ---------#
# ## riboseq
# orf_liver_3 = orf_TE %>% subset(Sample == "liver_3") %>% subset(orfID %in% orf_TE_in1$orfID) %>% select(orfID, ORF_type, transcript_id, f1Tpm)
...
#   theme_classic()
# 
# ggsave(file.path(plots_wd,"PNG/translation_efficiency_persample_liver.png"), height=4.72, width=6.02)
# ggsave(file.path(plots_wd,"PDF/translation_efficiency_persample_liver.pdf"), height=4.72, width=6.02)
# 
# ## if an ORF is translated in more than 1 sample, compute the mean
# mean_liver_TE = liver_TE %>%
#   group_by(orfID, gene_type, gene_id, gene_name, ORF_type) %>%
#   summarize(mean_TE = ifelse(n() > 1, mean(TE), TE))
...
#   theme(legend.position="none")
# 
# ggsave(file.path(plots_wd,"PNG/translation_efficiency_meanTE_liver.log10.png"), height=4.72, width=5.02)
# ggsave(file.path(plots_wd,"PDF/translation_efficiency_meanTE_liver.log10.pdf"), height=4.72, width=5.02)
# 
...
#   theme(legend.position="none")
# 
# ggsave(file.path(plots_wd,"PNG/translation_efficiency_meanTE_liver.png"), height=4.72, width=5.02)
# ggsave(file.path(plots_wd,"PDF/translation_efficiency_meanTE_liver.pdf"), height=4.72, width=5.02)
```
...
For those we consider as tissue specific (Testis-specific in this case), which is the RNASeq expression in the other tissues? We want to answer if there are genes expressed in all or both tissues but only translated in one.

```{r control_rnaseq}
TPMs_RNA_Testis_in1 = TPMs_RNA %>% subset(gene_id %in% ribORF_humanTestis_in1$transcript_id)
TPMs_RNA_Testis_in1 = TPMs_RNA_Testis_in1 %>% pivot_longer(cols=-c(gene_id, Length), names_to="sample", values_to = "TPM")
## get tissue
TPMs_RNA_Testis_in1$tissue = str_match(TPMs_RNA_Testis_in1$sample, "_\\s*(.*?)\\s*_rna")[,2]
## compute median
medianTPMs_RNA_Testis_in1 = TPMs_RNA_Testis_in1 %>% group_by(gene_id,tissue) %>% summarise(meanTPM = mean(TPM))
names(medianTPMs_RNA_Testis_in1)[1] = "transcript_id"
medianTPMs_RNA_Testis_in1 = merge(medianTPMs_RNA_Testis_in1, annot, by="transcript_id")
## log
medianTPMs_RNA_Testis_in1$logmeanTPM = log(medianTPMs_RNA_Testis_in1$meanTPM)
medianTPMs_RNA_Testis_in1 = medianTPMs_RNA_Testis_in1 %>% select(-meanTPM)
## 1 column per tissue
medianTPMs_RNA_Testis_in1_wide = medianTPMs_RNA_Testis_in1 %>% pivot_wider(names_from = tissue, values_from = logmeanTPM)

ggplot(medianTPMs_RNA_Testis_in1_wide %>% select(-liver), aes(x=testis, y=brain)) +
  geom_point(size=.4, aes(color=gene_type)) +
  ...
  geom_vline(xintercept = 1, color="red") +
    scale_color_manual(values=c("protein_coding"="#849DB1",
                        ...
                        "processed_pseudogene" = "#FBB04E",
                        "novel" = "#B09C85")) +
  labs(title="RNA expression Testis vs Brain of genes with translated ORFs",
       x="mean logTPM Testis",
       y="mean logTPM Brain") +
  theme_classic()

## without protein coding
ggplot(medianTPMs_RNA_Testis_in1_wide %>% select(-liver) %>% subset(gene_type != "protein_coding"), aes(x=testis, y=brain)) +
  geom_point(size=.4, aes(color=gene_type)) +
  geom_hline(yintercept = 1, color="red") +
  geom_vline(xintercept = 1, color="red") +
      scale_color_manual(values=c("protein_coding"="#849DB1",
...
ggplot(medianTPMs_RNA_Testis_in1_wide %>% select(-brain), aes(x=testis, y=liver)) +
  geom_point(size=.4, aes(color=gene_type)) +
  geom_hline(yintercept = 1, color="red") +
  geom_vline(xintercept = 1, color="red") +
      scale_color_manual(values=c("protein_coding"="#849DB1",
                        ...
                        "processed_pseudogene" = "#FBB04E",
                        "novel" = "#B09C85")) +
  labs(title="RNA expression Testis vs Liver of genes with translated ORFs",
       x="mean logTPM Testis",
       y="mean logTPM Liver") +
  theme_classic()

## without protein coding
ggplot(medianTPMs_RNA_Testis_in1_wide %>% select(-brain) %>% subset(gene_type != "protein_coding"), aes(x=testis, y=liver)) +
  geom_point(size=.4, aes(color=gene_type)) +
  geom_hline(yintercept = 1, color="red") +
  geom_vline(xintercept = 1, color="red") +
      scale_color_manual(values=c("protein_coding"="#849DB1",
...
#   scale_fill_manual(values=c("n_PCGs"="#849DB1","n_ncORFs" = "#FD8F24"), labels = c("ncORFs","CDS")) +
#   labs(x="Phylostratum",
#        y="Number of genes",
#        fill="Gene type",
...
# ggsave(file.path(plots_wd,"PNG/evoAge_translatedin1.png"), width=7.97, height=4.59)
# ggsave(file.path(plots_wd,"PDF/evoAge_translatedin1.pdf"), width=7.97, height=4.59)
```
...
#   theme(legend.position = "none") +
#   facet_wrap(~ gene_type)
# ggsave(file.path(plots_wd,"PNG/evoAge_vs_length_translatedin1.png"), width=7.97, height=4.59)
# ggsave(file.path(plots_wd,"PDF/evoAge_vs_length_translatedin1.pdf"), width=7.97, height=4.59)
```
...
# ggsave(file.path(plots_wd,"PNG/evoAge_translatedin1_biotype.png"), width=11.48, height=6.84)
# ggsave(file.path(plots_wd,"PDF/evoAge_translatedin1_biotype.pdf"), width=11.48, height=6.84)
```