...
for file in $INDIR/macaque*bam; do
    sample=${file%%.bam*}
    sample=${sample##*/}
...
INDIR=/datasets/marta/Wang2020/RNASeq
FQDIR=/users/genomics/marta/Wang2020_macaca/RNASeq/fastq
mkdir -p $FQDIR

QCDIR=/users/genomics/marta/Wang2020_macaca/RNASeq/QC
mkdir -p $QCDIR
...
BAMDIR=/users/genomics/marta/Wang2020_macaca/RNASeq/STAR

######################################################################################################
#####################################ALIGNMENT########################################################
GENOMEDIR=/users/genomics/saraa/projectTestis/genomes/macaca

ANNOTGENE=$GENOMEDIR
GNMIDX=$GENOMEDIR/Index_Genomes_STAR_2.8/Idx_Macaca_mulatta.Mmul_10_readlength75
...
STAR --runThreadN $SLURM_CPUS_PER_TASK --limitBAMsortRAM 50000000000 \
    ...
    --sjdbGTFfile /datasets/Common/ReferenceGenomes/Macaca/10.111/GenomesAndAnnotation/Macaca_mulatta.Mmul_10.111.gtf --twopassMode Basic
