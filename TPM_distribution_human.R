...
shared_tissue$tissue = sapply(strsplit(shared_tissue$sample, "_"), function(x) x[2])
shared_tissue = shared_tissue %>% 
  subset(gene_type == "lncRNA" | gene_type == "processed_pseudogene" | gene_type == "novel" | gene_type == "protein_coding") %>%
  subset(TPM > 1) %>%
  group_by(gene_id, gene_type, tissue) %>%
  summarise(num_patients=n())

shared_tissue_n = shared_tissue %>% group_by(gene_type, tissue, num_patients) %>% summarise(num_genes = n())

ggplot(shared_tissue_n, aes(x=num_patients, y=num_genes, color=gene_type)) +
  geom_point(stat="identity") +
  geom_line()+
  scale_y_continuous(trans="log10") +
  ggtitle("Shared genes, all samples | > 300 pb & > 1 TPM") +
