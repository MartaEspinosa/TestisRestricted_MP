...
```{r import, echo=F}
data_completed_TSA = read.csv(file.path(tumorReactDIR,"TSTR_Expression.csv"))
...
for (ctype_var in c("BRCA","BLCA","LUAD","KIRC","PRAD","LUSC","COAD","LIHC")) {
  ...
  ############ tumor - log2ratio 3x
  df = data_completed_TSA %>% subset(ctype == ctype_var) %>% subset(normal_tumor == "tumor")
  df = df %>% unique()
  ...
  lncRNA = df %>% subset(gene_type != "protein_coding") %>% select(gene_name) %>% unique() %>% subset(gene_name %in% df_expr$gene_name)
  CTA = df %>% subset(gene_type == "protein_coding") %>% select(gene_name) %>% unique() %>% subset(gene_name %in% df_expr$gene_name)
  chrX = df %>% subset(chr == "X") %>% select(gene_name) %>% unique() %>% subset(gene_name %in% df_expr$gene_name)
  chrA = df %>% subset(chr != "X") %>% select(gene_name) %>% unique() %>% subset(gene_name %in% df_expr$gene_name)
  ...
  save_pheatmap_pdf(cta_ph, paste0(heatmaps_wd,"/PDF/pheatmap_TOv3x5percent_log2ratio3x_CTA_",ctype_var,"_tumors.pdf"))
  ...
  save_pheatmap_pdf(lncRNA_ph, paste0(heatmaps_wd,"/PDF/pheatmap_TOv3x5percent_log2ratio3x_lncRNA_",ctype_var,"_tumors.pdf"))
  ...
  save_pheatmap_pdf(chrX_ph, paste0(heatmaps_wd,"/PDF/pheatmap_TOv3x5percent_log2ratio3x_chrX_",ctype_var,"_tumors.pdf"))
  ...
  save_pheatmap_pdf(chrA_ph, paste0(heatmaps_wd,"/PDF/pheatmap_TOv3x5percent_log2ratio3x_chrA_",ctype_var,"_tumors.pdf"))
