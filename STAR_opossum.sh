...
GENOMEDIR=/datasets/Common/ReferenceGenomes/Opossum/ASSM229v1/GenomesAndAnnotation
declare -a sample_array

for file in $INDIR/opossum*bam; do
    sample=${file%%.bam*}
    sample=${sample##*/}
...
INDIR=/datasets/marta/Wang2020/RNASeq
FQDIR=/users/genomics/marta/Wang2020_opossum/RNASeq/fastq
mkdir -p $FQDIR

QCDIR=/users/genomics/marta/Wang2020_opossum/RNASeq/QC
mkdir -p $QCDIR
...
BAMDIR=/users/genomics/marta/Wang2020_opossum/RNASeq/STAR

######################################################################################################
#####################################INDEX########################################################
ANNOTGENE=$GENOMEDIR
FASTADIR=$GENOMEDIR
temp_clusterDIR=/users/genomics/marta/star_indexes/opossum
...
STAR --runThreadN $SLURM_CPUS_PER_TASK --runMode genomeGenerate --genomeDir $GNMIDX --genomeFastaFiles $FASTADIR/Monodelphis_domestica.ASM229v1.dna.toplevel.fa --sjdbGTFfile $ANNOTGENE/Monodelphis_domestica.ASM229v1.112.chr.gtf --sjdbOverhang 74
...
STAR --runThreadN $SLURM_CPUS_PER_TASK --limitBAMsortRAM 50000000000 \
    ...
    --sjdbGTFfile $GENOMEDIR/Monodelphis_domestica.ASM229v1.112.chr.gtf --twopassMode Basic
