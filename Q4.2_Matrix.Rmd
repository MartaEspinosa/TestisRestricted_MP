...
mycolors = list(
  ...
  "gene_type" = c("protein_coding"="#849DB1",
                               "lncRNA" = "#B66353",
...
TSA_TestisGTEx_Translated = TSA_TestisGTEx_Translated %>% mutate(n_percent = case_when(ctype == "BRCA" ~ n/109,
                                                                                       ctype == "BLCA" ~ n/38,
                                                                                       ctype == "LUAD" ~ n/179,
                                                                                       ctype == "KIRC" ~ n/142,
                                                                                       ctype == "LUSC" ~ n/49,
...
```{r import, echo=F}
# load(file.path(save_wd,"Q4.2_matrix.RData"))
```
...
display_numbers_custom = ifelse(patient_ctype_mat_lncRNA_wide > 0.1, round(patient_ctype_mat_lncRNA_wide,2), "")

ph_patients = pheatmap(patient_ctype_mat_lncRNA_wide, 
         ...
         fontsize = 6,
         main="TSA lncRNA | percentage of patients (at least 5% in one cancer type)")
save_pheatmap_pdf(ph_patients, paste0(plots_wd,"/PDF/TTS_pheatmap_patients_5percent_lncRNA.pdf"))
...
# Numbers greater than 0.1 are kept, others are replaced with an empty string
display_numbers_custom = ifelse(patient_ctype_mat_CTA_wide > 0.1, round(patient_ctype_mat_CTA_wide,2), "")

ph_CTA_patients = pheatmap(patient_ctype_mat_CTA_wide, 
         clustering_distance_cols = "correlation",
         clustering_distance_rows = "euclidean", 
         clustering_method = "complete",
         display_numbers = display_numbers_custom,  # Only display numbers > 0.1
         ...
         color = colorRampPalette(c("lightyellow", "red"))(50),
         fontsize = 6,
         main="TSA CTA | percentage of patients (at least 5% in one cancer type)")
save_pheatmap_pdf(ph_CTA_patients, paste0(plots_wd,"/PDF/TTS_pheatmap_patients_5percent_CTA.pdf"))
...
data_completed_TSA_wide = data_completed_TSA_wide %>% filter(normal < 0.1 & tumor > 1) %>% select(-normal) %>% group_by(gene_name,ctype) %>% mutate(medianTPM = median(tumor)) %>% ungroup()
```
...
ph = pheatmap(nozeros_matrix_log, clustering_distance_cols = "correlation",clustering_distance_rows = "euclidean", clustering_method = "complete",
       ...
       color=colorRampPalette(c("lightyellow", "red"))(50),
       fontsize = 6,
       main=paste0("Median logTPM of Testis/Tumor Specific genes"))
save_pheatmap_pdf(ph, paste0(plots_wd,"/PDF/pheatmap_medianTPM_all.pdf"))
...
cta_ph = pheatmap(CTA_matrix_non_zero, clustering_distance_cols = "correlation",clustering_distance_rows = "euclidean", clustering_method = "complete",
       ...
       color=colorRampPalette(c("lightyellow", "red"))(50),
       fontsize = 6,
       main="Median logTPM of Testis/Tumor Specific genes | CTA")
save_pheatmap_pdf(cta_ph, paste0(plots_wd,"/PDF/pheatmap_medianTPM_CTA.pdf"))
...
lncRNA_ph = pheatmap(lncRNA_matrix_non_zero, clustering_distance_cols = "correlation",clustering_distance_rows = "euclidean", clustering_method = "complete",
       ...
       color=colorRampPalette(c("lightyellow", "red"))(50),
       fontsize = 6,
       main="Median logTPM of Testis/Tumor Specific genes | lncRNA")
save_pheatmap_pdf(lncRNA_ph, paste0(plots_wd,"/PDF/pheatmap_medianTPM_lncRNA.pdf"))
```
...
# })
  ...
  # plot(2:k_max, wss_values_all, type = "b", xlab = "Number of clusters", ylab = "Within-cluster sum of squares", main=paste0("all",ctype_var))
  ...
  ## we could check the quality of such clusters by using silhouette score
}
```
...
ggsave(file.path(plots_wd, "PNG/TTSburden_coding_vs_noncoding.png"))
ggsave(file.path(plots_wd, "PDF/TTSburden_coding_vs_noncoding.pdf"))
