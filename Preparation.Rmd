...
corr_TPMs = read.csv("/projects_eg/projects/marta/TestisRestricted_Microproteins_TSA/with_TranscriptomeReconstruction/Q1_TestisORFs/human/translationEfficiency_testis.withoutZeros.csv")

## sample1
corr_TPMs %>% subset(sample == "Testis_1") %>%
...
## sample2
corr_TPMs %>% subset(sample == "Testis_2") %>%
  mutate(log_RNA_TPM = log(RNA_TPM),
         log_Ribo_TPM = log(Ribo_TPM)) %>%
  ggplot(., aes(x=log_RNA_TPM, y=log_Ribo_TPM)) +
  geom_point() +
...
## sample3
corr_TPMs %>% subset(sample == "Testis_3") %>%
  mutate(log_RNA_TPM = log(RNA_TPM),
         log_Ribo_TPM = log(Ribo_TPM)) %>%
  ggplot(., aes(x=log_RNA_TPM, y=log_Ribo_TPM)) +
  geom_point() +
...
before = read.csv("/projects_eg/projects/marta/TestisRestricted_Microproteins_TSA/with_TranscriptomeReconstruction/Q2_TestisRestricted/human/testisRestricted_GTEx_translatedONLYtestis.csv")
...
```{r perpatient, echo=F}
candidates_per_patient = read.csv("/users/genomics/marta/TestisProject_SaraRazquin/with_TranscriptomeReconstruction/cancers/overexpression_tumor_3xnormal/cancertypes/GTEx/GTEx_tumor_3xnormal_TPMs_patients_ctypes.csv")
candidates_per_patient = merge(candidates_per_patient, tumorReact %>% select(gene_id, ctype, coding_noncoding) %>% unique(), by=c("gene_id","ctype"))
candidates_per_patient_counts = candidates_per_patient %>% group_by(coding_noncoding, gene_name, ctype) %>% count()

ggplot(candidates_per_patient_counts %>% subset(ctype != "KIRP"), aes(x=ctype, y=n, fill=coding_noncoding)) +
