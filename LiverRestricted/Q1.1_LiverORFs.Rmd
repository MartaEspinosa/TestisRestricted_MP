...
#### How many ORFs are translated in at least 1 out of the 3 liver samples?
...
ribORF_humanLiver_in1 = ribORF_humanLiver_Annotated_biotypes %>% subset(orfID %in% translated_in_2$orfID) 
write.csv(ribORF_humanLiver_in1, file.path(save_wd,"ribORF_humanLiver_translated_in_1.fulltable.csv"), row.names = F)

ribORF_humanLiver_in1 = ribORF_humanLiver_in1 %>% select(orfID, gene_id, gene_name, transcript_id, gene_type, orfType, length) %>% unique()
write.csv(ribORF_humanLiver_in1, file.path(save_wd,"ribORF_humanLiver_translated_in_1.simplified.csv"), row.names = F)

ORFs = as.data.frame(table(ribORF_humanLiver_in1$gene_type))
names(ORFs) = c("gene_type","num_ORFs")

## that correspond to:
ribORF_humanLiver_in1_genes = ribORF_humanLiver_in1 %>% select(gene_id, gene_type) %>% unique()
genes = as.data.frame(table(ribORF_humanLiver_in1_genes$gene_type))
...
num_ORFs_per_transcript = ribORF_humanLiver_in1 %>% 
...
ggplot(ribORF_humanLiver_in1, aes(x=gene_type, y=length, fill=gene_type)) +
  geom_boxplot() +
  scale_fill_manual(values=c("protein_coding"="#849DB1",
                         "lncRNA" = "#B66353",
                         "processed_pseudogene" = "#FBB04E",
...
wilcox.test(ribORF_humanLiver_in1$length ~ ribORF_humanLiver_in1$orfType)
```

```{r length_2, echo=F}
ribORF_humanLiver_in1$length_aa = ribORF_humanLiver_in1$length/3
ggplot(ribORF_humanLiver_in1, aes(x=length_aa, fill=gene_type, color="black")) +
...
ribORF_humanLiver_in1$start_codon = str_sub(ribORF_humanLiver_in1$orfID,-3)

ggplot(ribORF_humanLiver_in1, aes(x=gene_type, fill=start_codon)) +
...
## select those that are in subset of interest --> ribORF_humanLiver_in1
nrow(ribORF_humanLiver_in1)
fastaCandidates_subset = fastaCandidates %>% subset(orfID %in% ribORF_humanLiver_in1$orfID)
nrow(fastaCandidates_subset)

## add the fasta sequences to the dataframe
ribORF_humanLiver_in1_fastas = merge(fastaCandidates_subset, ribORF_humanLiver_in1, by="orfID")
nrow(ribORF_humanLiver_in1_fastas)

## translate the ORFs
ribORF_humanLiver_in1_fastas$ORFpep = sapply(ribORF_humanLiver_in1_fastas$ORFseq, translate_dna_to_aa)
write.csv(ribORF_humanLiver_in1_fastas, file.path(save_wd, "ribORF_humanLiver_in1.csv"), row.names = F)
ribORF_humanLiver_in1_fastas = read.csv(file.path(save_wd, "ribORF_humanLiver_in1.csv"))
```
...
## select ncORFs
ncORFs_seqs = ribORF_humanLiver_in1_fastas %>% subset(orfType == "noncoding")
ncORFs_seqs = as.character(ncORFs_seqs$ORFpep)

## ORFs
ORFs_seqs = ribORF_humanLiver_in1_fastas %>% subset(orfType == "canonical")
...
orf_TE_in1 = orf_TE %>% 
  ...
  unique() %>% 
  subset(orfID %in% ribORF_humanLiver_in1$orfID) %>% 
  unique()
nrow(orf_TE_in1)
...
#----------- sample 1 ---------#
## riboseq
orf_liver_1 = orf_TE %>% subset(Sample == "liver_1") %>% subset(orfID %in% orf_TE_in1$orfID) %>% select(orfID, ORF_type, transcript_id, f1Tpm)
...
# ribORF_humanBrain_2$sample = "Brain1"
# ## get transcriptID
