...
riboseq_dir = "/users/genomics/marta/TestisProject_SaraRazquin/with_TranscriptomeReconstruction/v47/human/RiboNovel_MultMap_isoforms"

```

```{r annotation, echo=F}
annot = read.csv("/users/genomics/marta/TestisProject_SaraRazquin/with_TranscriptomeReconstruction/v47/human/newReference_Resconstructed/transID_geneID_isoforms_selected.csv")
```
...
```{r ORFs, echo=F}
## get gene_id, gene_name, transcript_type
...
## how many orfTypes per transcript_type?
ribORF_humanTestis_Annotated %>% subset(transcript_type == "lncRNA" | transcript_type == "processed_pseudogene" | transcript_type == "protein_coding" | transcript_type == "novel") %>% group_by(transcript_type, orfType) %>% count()

## for protein-coding we only keep the canonical
ribORF_humanTestis_Annotated_biotypes = ribORF_humanTestis_Annotated %>%
  subset(transcript_type == "lncRNA" | transcript_type == "processed_pseudogene" | transcript_type == "protein_coding" | transcript_type == "novel")
table(ribORF_humanTestis_Annotated_biotypes$orfType, ribORF_humanTestis_Annotated_biotypes$transcript_type)

## understanding the protein coding ORFs
PCG_ORFs = ribORF_humanTestis_Annotated_biotypes %>% subset(transcript_type == "protein_coding") %>% select(transcript_id, gene_id, transcript_type, gene_name, orfID, orfType) %>% unique()
table(PCG_ORFs$gene_name)
...
for(g in 1:length(unique(ribORF_humanTestis_Annotated_biotypes$transcript_type))){
  temp_to_plot = ribORF_humanTestis_Annotated_biotypes %>% subset(transcript_type == unique(ribORF_humanTestis_Annotated_biotypes$transcript_type)[g])
  ggvenn(list("Sample1"=temp_to_plot %>% subset(sample == "Testis1") %>% pull(orfID),
       ...
       fill_alpha=0.5, stroke_color="gray", set_name_size = 4, text_size=3, fill_color = c("#7876B1","red","#91D1C2")) +
  ggtitle(paste0("Testis-expressed", unique(ribORF_humanTestis_Annotated_biotypes$transcript_type)[g]," ORFs"))
  ggsave(paste0(plots_wd,"/PNG/venn_TestisExpressedORFs_",unique(ribORF_humanTestis_Annotated_biotypes$transcript_type)[g],".png"))
  ggsave(paste0(plots_wd,"/PDF/venn_TestisExpressedORFs_",unique(ribORF_humanTestis_Annotated_biotypes$transcript_type)[g],".pdf"))
}
...
translated_in_1 = ribORF_humanTestis_Annotated_biotypes %>%
  select(orfID,gene_id,transcript_type,sample, orfType) %>%
  group_by(orfID,gene_id,transcript_type,orfType) %>%
  count() %>% 
  subset(n>=1) %>% unique()

table(translated_in_1$transcript_type)
table(translated_in_1$transcript_type, translated_in_1$orfType)
...
ribORF_humanTestis_in1 = ribORF_humanTestis_in1 %>% select(orfID, gene_id, gene_name, transcript_id, transcript_type, orfType, length) %>% unique()
write.csv(ribORF_humanTestis_in1, file.path(save_wd,"ribORF_humanTestis_translated_in_1.simplified.csv"), row.names = F)

ORFs = as.data.frame(table(ribORF_humanTestis_in1$transcript_type))
names(ORFs) = c("transcript_type","num_ORFs")

## that correspond to:
ribORF_humanTestis_in1_genes = ribORF_humanTestis_in1 %>% select(gene_id, transcript_type) %>% unique()
genes = as.data.frame(table(ribORF_humanTestis_in1_genes$transcript_type))
names(genes) = c("transcript_type","num_genes_w_ORFs")

ORFs_in_genes = merge(ORFs, genes, by="transcript_type")
ORFs_in_genes$ratio = ORFs_in_genes$num_ORFs / ORFs_in_genes$num_genes_w_ORFs
ORFs_in_genes

### geneORFtype
ribORF_humanTestis_in1 = ribORF_humanTestis_in1 %>% mutate(geneORFtype = paste0(transcript_type,"_",orfType))
```
...
ggplot(ribORF_humanTestis_in1, aes(x=reorder(geneORFtype, length), y=length)) +
  geom_boxplot() +
  # scale_fill_manual(values=c("protein_coding"="#CC79A7",
  #                        "lncRNA" = "#009E73",
  #                        "processed_pseudogene" = "#0090B2",
  ...
  facet_grid(~ transcript_type, scales="free_x", space="free")
...
ggplot(ribORF_humanTestis_in1, aes(x=length_aa, fill=transcript_type, color="black")) +
...
ggplot(ribORF_humanTestis_in1, aes(x=transcript_type, fill=start_codon)) +
...
mean_testis_TE = testis_TE %>% unique() %>% ungroup() %>%
  ...
  ungroup() %>% select(orfID, transcript_type, gene_id, gene_name, ORF_type, mean_TE) %>% unique()
write.csv(mean_testis_TE, file.path(save_wd, "mean_translationEfficiency_testis.csv"), row.names=F)

ggplot(mean_testis_TE, aes(x=transcript_type, y=log10(mean_TE))) +
  # geom_violin(aes(fill=transcript_type), alpha=.2) +
  geom_quasirandom(dodge.width = 0.9, size=1, alpha=.3,  varwidth = TRUE, aes(color=transcript_type)) +
  geom_boxplot(width=0.25, aes(color = "black", fill=transcript_type), alpha=.8) +
  scale_fill_manual(values=c("protein_coding"="#CC79A7",
                        "lncRNA" = "#009E73",
                        "processed_pseudogene" = "#0090B2",
                        "novel" = "#E69F00")) +
...
ggplot(mean_testis_TE, aes(x=transcript_type, y=mean_TE)) +
  # geom_violin(aes(fill=transcript_type), alpha=.2) +
  geom_quasirandom(dodge.width = 0.9, size=1, alpha=.3,  varwidth = TRUE, aes(color=transcript_type)) +
  geom_boxplot(width=0.25, aes(color = "black", fill=transcript_type), alpha=.8) +
  scale_fill_manual(values=c("protein_coding"="#CC79A7",
...
ggplot(mean_testis_TE, aes(x=transcript_type, y=log10(mean_TE))) +
  # geom_violin(aes(fill=transcript_type), alpha=.2) +
  geom_boxplot(aes(fill=transcript_type), alpha=.8, notch=T) +
  scale_fill_manual(values=c("protein_coding"="#CC79A7",
                        "lncRNA" = "#009E73",
                        "processed_pseudogene" = "#0090B2",
                        "novel" = "#E69F00")) +
...
## how many orfTypes per transcript_type?
# ribORF_humanBrain_Annotated %>% subset(transcript_type == "lncRNA" | transcript_type == "processed_pseudogene" | transcript_type == "protein_coding") %>% group_by(transcript_type, orfType) %>% count()

## for protein-coding we only keep the canonical
ribORF_humanBrain_Annotated_biotypes = ribORF_humanBrain_Annotated %>%
  subset(transcript_type == "lncRNA" | transcript_type == "processed_pseudogene" | transcript_type == "protein_coding" | transcript_type == "novel") %>%
  filter(!(transcript_type == "protein_coding" & orfType != "canonical"))
...
## how many orfTypes per transcript_type?
# ribORF_humanLiver_Annotated %>% subset(transcript_type == "lncRNA" | transcript_type == "processed_pseudogene" | transcript_type == "protein_coding") %>% group_by(transcript_type, orfType) %>% count()
