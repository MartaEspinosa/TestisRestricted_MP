...
GENOMEDIR=/datasets/Common/ReferenceGenomes/Chicken/GRCg7b/GenomesAndAnnotation
declare -a sample_array

for file in $INDIR/chicken*bam; do
    sample=${file%%.bam*}
    sample=${sample##*/}
...
INDIR=/datasets/marta/Wang2020/RNASeq
FQDIR=/users/genomics/marta/Wang2020_chicken/RNASeq/fastq
mkdir -p $FQDIR

QCDIR=/users/genomics/marta/Wang2020_chicken/RNASeq/QC
mkdir -p $QCDIR
...
BAMDIR=/users/genomics/marta/Wang2020_chicken/RNASeq/STAR

######################################################################################################
#####################################INDEX########################################################
ANNOTGENE=$GENOMEDIR
FASTADIR=$GENOMEDIR
temp_clusterDIR=/users/genomics/marta/star_indexes/chicken
...
STAR --runThreadN $SLURM_CPUS_PER_TASK --runMode genomeGenerate --genomeDir $GNMIDX --genomeFastaFiles $FASTADIR/Gallus_gallus.bGalGal1.mat.broiler.GRCg7b.dna.toplevel.fa --sjdbGTFfile $ANNOTGENE/Gallus_gallus.bGalGal1.mat.broiler.GRCg7b.112.chr.gtf --sjdbOverhang 74
...
STAR --runThreadN $SLURM_CPUS_PER_TASK --limitBAMsortRAM 50000000000 \
    ...
    --sjdbGTFfile $GENOMEDIR/Gallus_gallus.bGalGal1.mat.broiler.GRCg7b.112.chr.gtf --twopassMode Basic
