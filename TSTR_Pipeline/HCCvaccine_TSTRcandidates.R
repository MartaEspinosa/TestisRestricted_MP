...
immuno = immuno %>% select(Peptide, gene_id_x, source, matching_ORFpep)
names(immuno) = c("Peptide_broadSources","gene_id","BroadSource","ORFpep")
...
candORFs_HCC_SciAdv_MHCq = candORFs_HCC_SciAdv_MHCq %>% select(gene_id, gene_name, percentage_num_patients_overexpr, n_SciAdv, log2ratio, coding_noncoding_chr, source, sequence, BroadSource, Peptide_broadSources, orfID, accessions, ORFpep, everything()) %>% unique()
