...
candidates_complete = merge(gtf_candidates, candidates %>% select(-transcript_type), by=c("gene_name","gene_id","gene_type","chr","transcript_id"))

TSTR = candidates %>% select(gene_name, gene_id, geneORFtype, coding_noncoding_chr, ctype)
...
# 
  ...
  cnv_TSTR_ctype = merge(TSTR %>% subset(ctype == ctype_var), inside_start_end %>% select(gene_name, Unique_Name, Cytoband, Variant_Classification, Wide_Peak_Limits, position), by="gene_name")
  ...
  to_print = cnv_TSTR_ctype %>% select(Variant_Classification, gene_name, coding_noncoding_chr) %>% unique()
...
noncoding_genes = noncoding_results %>% select(gene_name, Variant_Classification, ctype, Tumor_Sample_Barcode) %>% unique() %>% group_by(gene_name, Variant_Classification, ctype) %>% count()
...
# print(unique(noncoding_results$gene_name))
to_table = ctnonx_results %>% select(gene_name, Variant_Classification, ctype) %>% unique() #%>% group_by(Variant_Classification, ctype) %>% count()
print(table(to_table$Variant_Classification, to_table$ctype))

# coding_genes = ctnonx_results %>% select(gene_name, Variant_Classification, ctype, num_patients_overexpr, percentage_num_patients_overexpr, Tumor_Sample_Barcode) %>% unique() %>% group_by(gene_name, Variant_Classification, ctype) %>% count()
...
byPatient = results %>% group_by(gene_name, coding_noncoding_chr, ctype, Variant_Classification) %>% count()
names(byPatient)[5] = "n_Gistic"

results_same_patients = merge(TOv3x_per_patient %>% select(patient_overexpr, ctype, gene_id), results, by.x = c("patient_overexpr","ctype","gene_id"), by.y = c("Tumor_Sample_Barcode","ctype","gene_id"))
results_same_patients = results_same_patients %>% unique()

temp = merge(patients_TPMs %>% select(patient, normal_tumor, TPM, ctype, gene_id), results, by.x = c("patient","ctype","gene_id"), by.y = c("Tumor_Sample_Barcode","ctype","gene_id"))

byMatchedPatient = results_same_patients %>% group_by(gene_name, coding_noncoding_chr, ctype, Variant_Classification) %>% count()
names(byMatchedPatient)[5] = "n_Matched"

summary_results = merge(byMatchedPatient, byPatient, by=c("gene_name", "coding_noncoding_chr", "ctype", "Variant_Classification"), all.y=T)
