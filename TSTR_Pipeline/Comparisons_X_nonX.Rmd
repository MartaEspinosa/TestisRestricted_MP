...
ggplot(candidatesORFs %>% 
         ...
         subset(grepl("ORF", geneORFtype)) %>% 
         unique(), 
       aes(x=factor(altORFs_chr, levels=c("other ncORFs-X","other ncORFs-nonX")), y=length_aa, fill=altORFs_chr)) +
  geom_boxplot() +
  geom_jitter() +
      scale_fill_manual(values=c("other ncORFs-X" = "#833437", "other ncORFs-nonX" = "#ead890")) +
  labs(y="Length (aa)",
...
ggplot(candidatesORFs %>% 
         select(-c(percentage_num_patients_overexpr, num_patients_overexpr, ctype)) %>% 
         subset(grepl("ORF", geneORFtype)) %>% 
         unique(), aes(x=factor(altORFs_chr, levels=c("other ncORFs-X","other ncORFs-nonX")), y=mean_TE, fill=altORFs_chr)) +
  geom_boxplot() +
      scale_fill_manual(values=c("other ncORFs-X" = "#833437", "other ncORFs-nonX" = "#ead890")) +
  labs(y="Mean Translation Efficiency in testis",
       x="",
