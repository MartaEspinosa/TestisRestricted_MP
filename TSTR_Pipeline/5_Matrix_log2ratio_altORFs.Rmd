...
TSTR_expr = merge(TSTR_expr, annot, by=c('transcript_id','gene_id','gene_name','gene_type'))
```
...
## get patients in which the gene is TSTR
step1 = merge(TSTR_expr %>% subset(normal_tumor == "tumor") %>% select(gene_id, patient, ctype, TPM), patients_data %>% select(gene_id,patient,ctype), by=c("gene_id","patient","ctype"))
step1 = step1 %>% subset(TPM > 1)
nrow(step1)
## add a forced 0 TPM in the patients where the gene is not TSTR
step2 = merge(step1, TSTR_expr %>% subset(normal_tumor == "tumor") %>% select(patient, ctype, gene_id, gene_name, gene_type, transcript_id, chr, Length, project), by=c("gene_id","patient","ctype"), all.y=T)
nrow(step2)
step2[is.na(step2)] = 0
...
# step1_2 = merge(TSTR_expr %>% subset(normal_tumor == "normal") %>% select(gene_id, patient, ctype, TPM), patients_data %>% select("gene_id","patient","ctype"), by=c("gene_id","patient","ctype"))
...
data_completed_TSA_forced = step2
...
## counting
counting_genes = data_completed_TSA_forced %>% select(gene_id, gene_type, ctype) %>% unique()
...
####### X / vs nonX
chrX = data_completed_TSA_wide %>% subset(chr == "X") %>% select(gene_name) %>% unique() %>% subset(gene_name %in% df_expr$gene_name)
nrow(chrX)
chrA = data_completed_TSA_wide %>% subset(chr != "X") %>% select(gene_name) %>% unique() %>% subset(gene_name %in% df_expr$gene_name)
nrow(chrA)
