...
png(file.path(plots_wd,"TOv3xlog2ratio3xMEAN_location_chr.png"), height=9.26, width=15.20, units = "in", res=400)
...
ggsave(file.path(plots_wd,"TSTR_per_ctype.jpeg"), height=4.94, width=6.85)
```
...
ggplot(candidatesORFs %>% 
  ...
  theme_minimal() 
ggsave(file.path(plots_wd,"logTPMTestis.jpeg"), height=5, width=6)
...
png(filename = file.path(plots_wd,"upset_TSTR_chr.png"), width=11.53, height=7.45, res=400, units="in")
ComplexUpset::upset(
...
ggplot(candidatesORFs %>% 
                subset(!grepl("ORF", geneORFtype)) %>%
         select(gene_id, gene_name, geneORFtype, median_mTEC, coding_noncoding_chr, type) %>% unique(), aes(x=factor(coding_noncoding_chr, levels=c("CT-X","CT-nonX","Noncoding-nonX","other ncORFs")),  y=(median_mTEC+0.001), color=coding_noncoding_chr)) +
  geom_violin() +
  ...
  theme_minimal() 
ggsave(file.path(plots_wd,"TPM_thymus.jpeg"))
...
ggplot(candidatesORFs %>% select(-c(ctype)) %>% 
  ...
  theme_minimal() +
  theme(legend.position = "none") 
ggsave(file.path(plots_wd,"lengthORF_geneORFtype.jpeg"), height = 5, width = 6)
...
ggsave(file.path(plots_wd,"lengthORF_altORFs.jpeg"), height = 5, width = 6)
```
...
ggplot(candidatesORFs %>% 
  ...
  theme_minimal() +
  theme(legend.position = "none")
ggsave(file.path(plots_wd,"TE_geneORFtype.jpeg"), height=5, width=6)
...
ggplot(candidatesORFs %>%
  ...
  theme_minimal() +
    scale_x_discrete(labels=c('dORFs', 'odORFs', 'ouORFs', 'uORFs')) 
ggsave(file.path(plots_wd,"TE_altORFs.jpeg"), height=5, width=6)
...
ggplot(candidatesORFs %>% 
  ...
  theme_minimal() +
  theme(legend.position = "none")
ggsave(file.path(plots_wd,"TranslationLevel_geneORFtype.jpeg"), height = 5, width = 6)
...
ggplot(candidatesORFs %>% 
  ...
  scale_x_discrete(labels=c('dORFs', 'odORFs', 'ouORFs', 'uORFs')) +
  theme_minimal() 
ggsave(file.path(plots_wd,"TranslationLevel_altORFs.jpeg"), height = 5, width = 6)
...
ggplot(candidatesORFs %>% 
  ...
  scale_x_discrete(labels=c('dORFs', 'odORFs', 'ouORFs', 'uORFs')) +
  theme_minimal() 
ggsave(file.path(plots_wd,"canonicalRatio_altORFs.jpeg"), width=6, height = 5)
```
...
#   theme_minimal() 
# ggsave(file.path(plots_wd,"Evo_geneORFtype.jpeg"))
# 
# candidatesORFs %>% 
...
png(file.path(plots_wd,"AA_radarchart_geneORFtype_noaltORFs.png"))
...
png(file.path(plots_wd,"AA_radarchart_ORFtype_withaltORFs.png"))
radarchart(to_plot, maxmin = T,
           axistype = 0, plwd=2 , plty=1,
           pcol=colors_border, title="Aminoacids enrichment in ORFs")
legend(x=-1.5, y=-0.8, legend = rownames(to_plot[-c(1,2),]), bty = "n", pch=20 , col=colors_border , text.col = "grey", cex=1, pt.cex=1.5)
...
png(file.path(plots_wd,"AA_radarchart_ORFNature.png"))
radarchart(to_plot, maxmin = T,
           axistype = 0, plwd=2 , plty=1,
           pcol=colors_border, title="Aminoacids enrichment in ORFs")
legend(x=-1.5, y=-0.8, legend = rownames(to_plot[-c(1,2),]), bty = "n", pch=20 , col=colors_border , text.col = "grey", cex=1, pt.cex=1.5)
...
png(file.path(plots_wd,"AA_radarchart_ncORFs.png"))
...
```{r stage, echo=F}
tumorexpression = read.csv(file.path(tumorReactDIR,"TSTR_Expression_tumor.csv"))
...
tumorexpression_candidates = tumorexpression %>% subset(patient %in% stage$case_submitter_id)
candidates_expression_pancancer = merge(tumorexpression_candidates, candidatesORFs %>% select(gene_id, gene_name, coding_noncoding_chr) %>% unique(), by=c("gene_name","gene_id"))
tumorexpression_candidates_1TPM = candidates_expression_pancancer %>% subset(TPM > 1)
tumorexpression_candidates_1TPM_stage = merge(tumorexpression_candidates_1TPM, stage, by.x="patient", by.y="case_submitter_id")

### CT in general
# Step 1: Count unique genes per patient
gene_count_per_patient <- tumorexpression_candidates_1TPM_stage %>%
  group_by(patient) %>%
  summarise(
    gene_count = n_distinct(gene_name),
    age_at_index = as.numeric(dplyr::first(as.vector(age_at_index))),
    ctype = dplyr::first(as.character(ctype)),
    stage = dplyr::first(as.character(stage))
  )
# Step 2: Summarize by stage
gene_count_by_stage <- gene_count_per_patient %>%
  group_by(stage) %>%
  summarise(
    avg_gene_count = mean(gene_count),
    age_adjusted = mean(age_at_index) # Optional if you want a quick view of age effect
  )
# Step 3: Adjust for age and cancer type using a GLM
# Fit a model to adjust gene counts
glm_model <- glm(
  gene_count ~ stage + age_at_index + ctype,
  data = gene_count_per_patient,
  family = poisson()  # Poisson is appropriate for count data
)
summary(glm_model)


gene_count_per_patient %>% 
  ggplot(., aes(x=stage, y=gene_count, fill=stage)) +
  geom_boxplot() +
  geom_jitter()
...
# Step 1: Count unique genes per patient
gene_count_per_patient <- tumorexpression_candidates_1TPM_stage %>%
  group_by(patient, coding_noncoding_chr) %>%
  summarise(
    gene_count = n_distinct(gene_name),
    age_at_index = as.numeric(dplyr::first(as.vector(age_at_index))),
    ctype = dplyr::first(as.character(ctype)),
...
# Step 2: Summarize by stage
gene_count_by_stage <- gene_count_per_patient %>%
  group_by(stage, coding_noncoding_chr) %>%
  summarise(
    avg_gene_count = mean(gene_count),
    age_adjusted = mean(age_at_index) # Optional if you want a quick view of age effect
  )
...
gene_count_per_patient %>% 
  ...
  geom_boxplot() +
  # geom_jitter() +
  facet_grid(ctype ~ coding_noncoding_chr)
