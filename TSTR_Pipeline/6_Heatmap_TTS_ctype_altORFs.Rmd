...
data_completed_TSA_forced = data_completed_TSA %>% select(-TPM)
data_completed_TSA_wide = data_completed_TSA_forced %>% subset(forced_TPM > 0)
...
data_completed_TSA_wide = data_completed_TSA_wide %>% group_by(gene_name,ctype) %>% mutate(medianTPM = median(forced_TPM)) %>% ungroup()

```

```{r import_TTS, echo=F}
data_completed_TSA_normals = read.csv(file.path(tumorReactDIR,"TSTR_Expression_normal.csv"))
data_completed_TSA_forced_normals = data_completed_TSA_normals %>% select(-TPM)
data_completed_TSA_forced_normals = merge(data_completed_TSA_forced_normals, annot, by=c("transcript_id","gene_id","gene_name","gene_type"))
...
# for (ctype_var in c("BLCA")) {
  ...
  df = data_completed_TSA_forced %>% subset(ctype == ctype_var) %>% subset(gene_id %in% candidates_ctype$gene_id)
  ...
  if (dim(df_5percent)[1] > 0) {
    ...
    df_5percent_expr = df_5percent %>% select(gene_name, patient, forced_TPM) %>% pivot_wider(names_from = patient, values_from = forced_TPM) %>% unique()
  ...
  if (length(gene_names_ctype) > 4) {
    ...
    ##### ----> CORRPLOT 4 - CTA-X chr <----- #####
          p <- {
    corrplot(CTAX_cormat, type = "lower", order = "hclust", method="circle",
             tl.col = "black", col = COL2('BrBG', 10), tl.cex=0.6, title=paste0("Correlation nonCTA-X chr TTS genes",  ctype_var),  mar=c(0,0,1,0));
      recordPlot() 
    }
    
    png(paste0(plots_wd,"/PNG/corrplot_nonCTAX_TOv3x_",ctype_var,".png"))
    replayPlot(p)
    dev.off()
    pdf(paste0(plots_wd,"/PDF/corrplot_nonCTAX_TOv3x_",ctype_var,".pdf"))
    replayPlot(p)
    dev.off()
  ...
  if (length(gene_names_ctype) > 4) {
    ...
    p <- {
    corrplot(CTAnonX_cormat, type = "lower", order = "hclust", method="circle",
             tl.col = "black", col = COL2('BrBG', 10), tl.cex=0.6, title=paste0("Correlation nonCTA-nonX chr TTS genes",  ctype_var),  mar=c(0,0,1,0));
      recordPlot() 
    }
    
    png(paste0(plots_wd,"/PNG/corrplot_nonCTAnonX_TOv3x_",ctype_var,".png"))
    replayPlot(p)
    dev.off()
    pdf(paste0(plots_wd,"/PDF/corrplot_nonCTAnonX_TOv3x_",ctype_var,".pdf"))
    replayPlot(p)
    dev.off()
  }
