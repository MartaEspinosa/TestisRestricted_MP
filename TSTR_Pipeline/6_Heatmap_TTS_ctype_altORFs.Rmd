...
#   ##############################################################################  
#   ##############################################################################
#     df = data_completed_TSA_normals %>% select(-forced_TPM) %>% subset(ctype == ctype_var) %>% subset(gene_name %in% rownames(tumor_matrix)) %>% subset(patient %in% colnames(tumor_matrix))
#   
#     df = df %>% unique()
# 
#     ## keep expression data to plot
...
#   ##############################################################################  
#   ##############################################################################
#     df = data_completed_TSA_normals %>% subset(ctype == ctype_var) %>% subset(gene_name %in% rownames(tumor_matrix)) %>% subset(patient %in% colnames(tumor_matrix))
#   
#     df = df %>% unique()
# 
#     ## keep expression data to plot
...
# 
#     ## keep expression data to plot
#     df_expr = df %>% select(gene_name, patient, forced_TPM) %>% pivot_wider(names_from = patient, values_from = forced_TPM) %>% unique()
#     df_expr = as.data.frame(df_expr)
#   
#     df_expr_values = mutate_all(df_expr[,-1], function(x) as.numeric(as.character(x)))
#     ## get log2TPM
