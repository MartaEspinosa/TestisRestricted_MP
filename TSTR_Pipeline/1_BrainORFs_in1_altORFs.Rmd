...
plots_wd = "/projects_eg/projects/marta/TestisRestricted_Microproteins_TSA/with_TranscriptomeReconstruction/Multimap_altORFs/Q1_TestisORFs/human/plots/brain"
save_wd = "/projects_eg/projects/marta/TestisRestricted_Microproteins_TSA/with_TranscriptomeReconstruction/Multimap_altORFs/Q1_TestisORFs/human/brain"
...
## Evidence of translation in brain according to the Ribo-Seq data in Wang et al. (2020) 
...
- Only reproducible results on translated ORFs are expected, so we only keep ORFs that are translated in 1 out of 3 brain samples
...
##-------------------------- SAMPLE 2 --------------------------##
ribORF_humanbrain_2 = read.table(file.path(riboseq_dir,"RibORF/human_brain_ribo_2_r1/pred.pvalue.parameters.txt"), header=T)

repre_ribORF_humanbrain_2 =  read.table(file.path(riboseq_dir,"RibORF/human_brain_ribo_2_r1/repre.valid.ORF.genepred.txt"), header=F)
...
ggvenn(list("Sample1"=ribORF_humanbrain_1$orfID,
                 "Sample2"=ribORF_humanbrain_2$orfID,
...
ggvenn(list("Sample1"=ribORF_humanbrain_1 %>% subset(grepl("TCONS",transcript_id)) %>% pull(orfID),
                 "Sample2"=ribORF_humanbrain_2%>% subset(grepl("TCONS",transcript_id)) %>% pull(orfID),
...
# nrow(ribORF_humanbrain_1_Annotated)
## get orfType
...
table(ribORF_humanbrain_Annotated_biotypes$orfType, ribORF_humanbrain_Annotated_biotypes$gene_type)

## understanding the protein coding ORFs
PCG_ORFs = ribORF_humanbrain_Annotated_biotypes %>% subset(gene_type == "protein_coding") %>% select(transcript_id, gene_id, gene_type, gene_name, orfID, orfType) %>% unique()
table(PCG_ORFs$orfType)

ribORF_humanbrain_Annotated_biotypes = ribORF_humanbrain_Annotated_biotypes %>% 
  subset(orfType == "uORF" | orfType == "dORF" | orfType == "ouORF" | orfType == "odORF" | orfType == "noncoding" | orfType == "canonical")


for(g in 1:length(unique(ribORF_humanbrain_Annotated_biotypes$gene_type))){
  temp_to_plot = ribORF_humanbrain_Annotated_biotypes %>% subset(gene_type == unique(ribORF_humanbrain_Annotated_biotypes$gene_type)[g])
  ggvenn(list("Sample1"=temp_to_plot %>% subset(sample == "brain1") %>% pull(orfID),
                 "Sample2"=temp_to_plot %>% subset(sample == "brain2") %>% pull(orfID),
                 "Sample3"=temp_to_plot %>% subset(sample == "brain3") %>% pull(orfID)),
       fill_alpha=0.5, stroke_color="gray", set_name_size = 4, text_size=3, fill_color = c("#7876B1","red","#91D1C2")) +
  ggtitle(paste0("brain-expressed", unique(ribORF_humanbrain_Annotated_biotypes$gene_type)[g]," ORFs"))
  ggsave(paste0(plots_wd,"/PNG/venn_brainExpressedORFs_",unique(ribORF_humanbrain_Annotated_biotypes$gene_type)[g],".png"))
  ggsave(paste0(plots_wd,"/PDF/venn_brainExpressedORFs_",unique(ribORF_humanbrain_Annotated_biotypes$gene_type)[g],".pdf"))
}
```

#### How many ORFs are translated in at least 1 out of the 3 brain samples?

ncORFs from lncRNA and processed pseudogenes + canonical ORFs.

```{r translated_in_1, echo=F}
## of the selected, select those that are translated in at least 2 out of 3 brain samples
...
translated_in_1_genes = ribORF_humanbrain_Annotated_biotypes %>%
...
### geneORFtype
ribORF_humanbrain_in1 = ribORF_humanbrain_in1 %>% mutate(geneORFtype = paste0(gene_type,"_",orfType))
```
...
Questions: How many ncORFs in lncRNAs/processed pseudogenes are translated in brain? Which is their length, composition and translation level in comparison to canonical ORFs? 
...
ggplot(ribORF_humanbrain_in1, aes(x=reorder(geneORFtype, length), y=length, fill=geneORFtype)) +
  geom_boxplot() +
...
ggplot(ribORF_humanbrain_in1, aes(x=geneORFtype, fill=start_codon)) +
  geom_bar(position="fill") +
  labs(x="Biotype",
       fill="Starting codon",
       title="Starting codon of ncORFs and canonical ORFs") +
...
TE_dir=
orf_TE_1 = read.csv("/users/genomics/marta/TestisProject_SaraRazquin/JC_RiboSeq_TPMs/Outputs/with_TranscriptomeReconstruction/Expression/v47_altORFs_MultMap/ribORFsimp.brain_1.tsv", sep="\t")
orf_TE_2 = read.csv("/users/genomics/marta/TestisProject_SaraRazquin/JC_RiboSeq_TPMs/Outputs/with_TranscriptomeReconstruction/Expression/v47_altORFs_MultMap/ribORFsimp.brain_2.tsv", sep="\t")
orf_TE_3 = read.csv("/users/genomics/marta/TestisProject_SaraRazquin/JC_RiboSeq_TPMs/Outputs/with_TranscriptomeReconstruction/Expression/v47_altORFs_MultMap/ribORFsimp.brain_3.tsv", sep="\t")
...
nrow(ribORF_humanbrain_in1)
orf_TE_in1 = merge(orf_TE, ribORF_humanbrain_in1 %>% select(orfID, geneORFtype, gene_id, gene_name), by="orfID")
nrow(orf_TE_in1)
```
...
#----------- sample 1 ---------#
## riboseq
orf_brain_1 = orf_TE %>% subset(Sample == "brain_1") %>% subset(orfID %in% orf_TE_in1$orfID) %>% select(orfID, transcript_id, f1Tpm)
names(orf_brain_1)[3] = "Ribo_TPM"
orf_brain_1 = merge(orf_brain_1, orf_TE_in1 %>% select(orfID, geneORFtype, gene_id, gene_name, transcript_id), by=c("orfID","transcript_id"))
...
#----------- sample 2 ---------#
## riboseq
orf_brain_2 = orf_TE %>% subset(Sample == "brain_2") %>% subset(orfID %in% orf_TE_in1$orfID) %>% select(orfID, transcript_id, f1Tpm)
names(orf_brain_2)[3] = "Ribo_TPM"
orf_brain_2 = merge(orf_brain_2, orf_TE_in1 %>% select(orfID, geneORFtype, gene_id, gene_name, transcript_id), by=c("orfID","transcript_id"))

## rnaseq
rna_brain_2 = brain_TPMs_with_ORFs %>% select(transcript_id, human_brain_rna_2)
names(rna_brain_2)[2] = "RNA_TPM"

brain_2 = merge(rna_brain_2, orf_brain_2, by=c("transcript_id"))

## TE
brain_2$TE = brain_2$Ribo_TPM / brain_2$RNA_TPM
brain_2$sample = "brain_2"

#----------- sample 3 ---------#
## riboseq
orf_brain_3 = orf_TE %>% subset(Sample == "brain_3") %>% subset(orfID %in% orf_TE_in1$orfID) %>% select(orfID, transcript_id, f1Tpm)
names(orf_brain_3)[3] = "Ribo_TPM"
orf_brain_3 = merge(orf_brain_3, orf_TE_in1 %>% select(orfID, geneORFtype, gene_id, gene_name, transcript_id), by=c("orfID","transcript_id"))
...
#--------------- merging ---------------#
brain_TE = rbind(brain_1, brain_2, brain_3)
...
```{r TE_plot, echo=F}
ggplot(brain_TE, aes(x=sample, y=TE, fill=geneORFtype)) +
  geom_boxplot() +
  scale_fill_manual(values=c("protein_coding_canonical"="#ead890",
                         "lncRNA_noncoding" = "#cd4f38",
                         "processed_pseudogene_noncoding" = "#e48c2a",
...
brain_TE = as.data.frame(brain_TE)
brain_TE$TE = as.numeric(brain_TE$TE)
mean_brain_TE = brain_TE %>% unique() %>% ungroup() %>%
  group_by(orfID) %>% 
...
ggplot(mean_brain_TE, aes(x=geneORFtype, y=log10(mean_TE))) +
...
ggplot(mean_brain_TE, aes(x=geneORFtype, y=mean_TE)) +
...
## Of the translated, how many are NOT translated in liver or testis?
...
##-------------------------- SAMPLE 2 --------------------------##
ribORF_humantestis_1 = read.table(file.path(riboseq_dir,"RibORF/human_testis_ribo_2_r1/pred.pvalue.parameters.txt"), header=T)

repre_ribORF_humantestis_1=  read.table(file.path(riboseq_dir,"RibORF/human_testis_ribo_2_r1/repre.valid.ORF.genepred.txt"), header=F)
...
### Concatenate ncORFs for all 2 samples
ribORF_humantestis = rbind(ribORF_humantestis_3, ribORF_humantestis_1, ribORF_humantestis_1)
...
CTDB = CTDB %>% mutate(X_A = case_when(grepl("^X", chr) ~ "X",
                                       TRUE ~ "A"))
print(nrow(CTDB))
print(nrow(ribORF_humanbrain_in1 %>% subset(gene_name %in% CTDB$gene_name)))
CTDB_translated_in_brain = merge(ribORF_humanbrain_in1, CTDB, by="gene_name")
table(CTDB_translated_in_brain$X_A)
...
notFound = CTDB %>% subset(!gene_name %in% CTDB_translated_in_brain$gene_name)
print(nrow(notFound))
