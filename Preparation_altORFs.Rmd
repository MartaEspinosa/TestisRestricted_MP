...
filtered_data_TAA_long = read.csv(file.path(pre_save_wd,"TAAdata_long.csv"))
filtered_data_TOv3x_long = filtered_data_TAA_long %>% subset(gene_name %in% tumorReact$gene_name)
tumorReact_full = merge(filtered_data_TOv3x_long, tumorReact %>% select(gene_name, length_aa, start_codon, ORFpep, orfType, geneORFtype), by=c("gene_name"))

tumorReact_full = merge(tumorReact_full, total_num_patients, by="ctype")
