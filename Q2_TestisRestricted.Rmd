...
gene_transcript = read.table("/genomics/users/marta/genomes/1transcript_1gene.txt",sep=",", header=T)
...
```{r annotation, echo=F}
annot<-read.table("/users/genomics/marta/TestisProject_SaraRazquin/with_TranscriptomeReconstruction/human/newReference_Resconstructed/1transcript_1gene.reconstructed.csv", header=F, sep=",")
annot<- annot %>% mutate(gene_type = case_when(grepl("STRG",V3) ~ "novel",
                                               TRUE ~ V3))
## novels
novels = annot %>% subset(gene_type == "novel")
novels$gene_id = novels$V2
novels$transcript_id = novels$V1
novels = novels %>% select(-c(V1,V2,V3))
novels$gene_name = novels$gene_id

## annotated
annotated = annot %>% subset(gene_type != "novel")
annotated$transcript_id = gsub("\\..*","", annotated$V2)
annotated$gene_id = gsub("\\..*","", annotated$V1)
annotated = annotated %>% select(-c(V1,V2,V3))
annotated = merge(annotated, gene_transcript, by=c("gene_id","gene_type"))

annotation = rbind(annotated, novels)
write.csv(annotation, "/users/genomics/marta/TestisProject_SaraRazquin/with_TranscriptomeReconstruction/human/newReference_Resconstructed/1transcript_1gene.reconstructed.csv", row.names = F)
head(annotation)
...
toc_TPMs = read.csv("/users/genomics/marta/TestisProject_SaraRazquin/with_TranscriptomeReconstruction/human/featureCounts_gffcompare/table_of_counts_TPMs.csv", header=TRUE)

# ## The names of the samples are simplified
# names(raw_counts) = sub(".*X.users.genomics.saraa.projectTestis.STAR.(.*)_r1Aligned.sortedByCoord.out.bam","\\1", names(raw_counts))
# names(raw_counts)[1] = "gene_id"
# raw_counts$gene_id = gsub("\\..*","",raw_counts$gene_id)
# dim(raw_counts)
# 
# ## Add gene_name and gene_type
# raw_counts_Symb = merge(raw_counts, gene_transcript, by="gene_id")
dim(toc_TPMs)
```
...
toc_TPMs_long$patient = str_sub(toc_TPMs_long$sample, start= -4, end=-4)
...
ggplot(toc_TPMs_long %>% subset(gene_type == "lncRNA" | gene_type == "processed_pseudogene" | gene_type == "protein_coding"), aes(x=patient, y=log2TPM, fill=gene_type))+
  geom_boxplot() +
  labs(x="Patient",
       fill="Biotype",
...
toc_TPMs_subset = toc_TPMs %>% subset(gene_type == "lncRNA" | gene_type == "processed_pseudogene" | gene_type == "protein_coding") %>% unique()

#------------------ BRAIN ------------------#
## they should be brain-expressed in at least 2 patients
brainexpr = toc_TPMs_subset[rowSums(toc_TPMs_subset[, c(3:5)] > 1) > 1, ]
## merge with all info
brainexpr_full = merge(brainexpr, toc_TPMs_long %>% select(gene_id, gene_name, gene_type, Length), by=c("gene_id","gene_type","gene_name","Length")) 
...
liverexpr = toc_TPMs_subset[rowSums(toc_TPMs_subset[, c(6:8)] > 1) > 1, ]
## merge with all info
liverexpr_full = merge(liverexpr, toc_TPMs_long %>% select(gene_id, gene_name, gene_type, Length), by=c("gene_id","gene_type","gene_name","Length")) 
...
testisexpr = toc_TPMs_subset[rowSums(toc_TPMs_subset[, c(9:11)] > 1) > 1, ]

## merge with all info
testisexpr_full = merge(testisexpr, toc_TPMs_long %>% select(gene_id, gene_name, gene_type, Length), by=c("gene_id","gene_type","gene_name","Length")) 
...
* CRITERIA 2: TPM > 1 in at least 2 testis sample and TPM < 0.5 in all other tissues 
...
brainspecific = toc_TPMs_subset[rowSums(toc_TPMs_subset[, c(3:5)] > 1) > 1 & rowSums(toc_TPMs_subset[, c(6:11)] < 0.5) == 6, ]
## merge with all info
brainspecific_full = merge(brainspecific, toc_TPMs_long %>% select(gene_id, gene_name, gene_type, Length), by=c("gene_id","gene_type","gene_name","Length")) 
brainspecific_full = brainspecific_full %>% unique()

# brainspecific_full_in2 = brainspecific_full[rowSums(brainspecific_full[, c(5:7)] > 1) > 1,]
...
liverspecific = toc_TPMs_subset[rowSums(toc_TPMs_subset[, c(6:8)] > 1) > 1 & rowSums(toc_TPMs_subset[, c(3:5,9:11)] < 0.5) == 6, ]
## merge with all info
liverspecific_full = merge(liverspecific, toc_TPMs_long %>% select(gene_id, gene_name, gene_type, Length), by=c("gene_id","gene_type","gene_name","Length")) 
liverspecific_full = liverspecific_full %>% unique()

# liverspecific_full_in2 = liverspecific_full[rowSums(liverspecific_full[, c(8:10)] > 1) > 1,]
...
testisspecific = toc_TPMs_subset[rowSums(toc_TPMs_subset[, c(9:11)] > 1) > 1 & rowSums(toc_TPMs_subset[, c(3:8)] < 0.5) == 6, ]

## merge with all info
testisspecific_full = merge(testisspecific, toc_TPMs_long %>% select(gene_id, gene_name, gene_type, Length), by=c("gene_id","gene_type","gene_name","Length")) 
testisspecific_full = testisspecific_full %>% unique()

# testisspecific_full_in2 = testisspecific_full[rowSums(testisspecific_full[, c(11:13)] > 1) > 1,]
...
gtex = read.table("/genomics/users/marta/genomes/GTEx/GTEx_maintissue_gene_median_tpm.csv", sep=",", header=T)
...
# merged_testis_gtex = merge(gtex, testisspecific_full, by="gene_id")
dim(testis_gtex)
...
write.csv(subset_testis_GTEx, file.path(save_wd,"testisRestricted_GTEx.csv"), row.names = F)
nrow(subset_testis_GTEx)
subset_testis_GTEx = na.omit(subset_testis_GTEx)
tmp = subset_testis_GTEx %>% select(gene_id, gene_type) %>% unique()
table(tmp$gene_type)
```
...
ggplot(GTEx_expressed_restricted, aes(x=gene_type, y=n, fill=testisRestricted_GTEx)) +
  geom_bar(stat="identity", position="fill") +
  geom_text(aes(label=paste0(perc, "%")), position=position_fill(vjust=0.5)) +
  scale_fill_manual(values=c("yes" = "#B1283A", "no"="#A8A6A7")) +
  scale_y_continuous(labels = scales::percent) +
...
subset_liver_full_GTEx = subset_liver_full_GTEx %>% subset(GTEx_healthy == "< 0.5") %>% subset(GTEx_liver == "> 0.5")
write.csv(subset_liver_full_GTEx, file.path(save_wd,"liverRestricted_GTEx.csv"), row.names = F)
nrow(subset_liver_full_GTEx)
tmp = subset_liver_full_GTEx %>% select(gene_id, gene_type) %>% unique()
table(tmp$gene_type)
```
...
write.csv(subset_brain_GTEx, file.path(save_wd,"brainRestricted_GTEx.csv"), row.names = F)
nrow(subset_brain_GTEx)
tmp = subset_brain_GTEx %>% select(gene_id, gene_type) %>% unique()
table(tmp$gene_type)
```
...
ggpie(to_plot, "n", label="n", fill="gene_type", lab.pos = "in", 
...
testisRestrictedGTEx_Translated = merge(subset_testis_GTEx %>% select(gene_id, gene_type, gene_name), ribORF_humanTestis_in2_fastas, by=c("gene_id","gene_name","gene_type")) 
...
ggpie(to_plot, "n", label="n", fill="gene_type", lab.pos = "in", 
      palette=c("protein_coding"="#849DB1",
                 "lncRNA" = "#B66353",
                 "processed_pseudogene" = "#FBB04E")) +
  ggtitle("ORFs from testis-Restricted GTEx genes only translated in testis (2 samples at least)")
...
```{r differences_Restricted, echo=F}
# #### should I do it vs all or vs the non-testisRestricted?
# ##---------- length differences ----------##
# ggplot(testisRestrictedGTEx_Translated, aes(x=testisRestricted_GTEx, y=length_aa, fill=testisRestricted_GTEx)) +
#   geom_boxplot() +
#   stat_compare_means() +
#   scale_y_continuous(trans="log10") +
#     scale_fill_manual(values=c("yes" = "#B1283A", "no"="#A8A6A7")) +
#   labs(title="Translated TestisRestricted GTEx ORFs") +
#   theme_classic()
# ggsave(file.path(plots_wd,"PNG/testisExpressed_testisRestrictedGTEx_lengthAA.png"), height=4.69, width=5.4)
# ggsave(file.path(plots_wd,"PDF/testisExpressed_testisRestrictedGTEx_lengthAA.pdf"), height=4.69, width=5.4)
# 
# ##---------- length differences | gene_type ----------##
# ggplot(testisRestrictedGGTEx_Translated, aes(x=gene_type, y=length_aa, fill=testisRestricted)) +
#   geom_boxplot() +
#   stat_compare_means(label="p.signif") +
#   scale_y_continuous(trans="log10") +
#   scale_fill_manual(values=c("yes" = "#B1283A", "no"="#A8A6A7")) +
...
# 
# ## select ncORFs - no testisRestricted
# ncORFs_no_seqs = testisRestrictedGGTEx_Translated %>% subset(orfType == "noncoding") %>% subset(testisRestricted == "no")
# ncORFs_no_seqs = as.character(ncORFs_no_seqs$ORFpep)
# ## select ncORFs - testisRestricted
# ncORFs_yes_seqs = testisRestrictedGGTEx_Translated %>% subset(orfType == "noncoding") %>% subset(testisRestricted == "yes")
# ncORFs_yes_seqs = as.character(ncORFs_yes_seqs$ORFpep)
# 
# ## select ORFs - no testisRestricted
# ORFs_no_seqs = testisRestrictedGGTEx_Translated %>% subset(orfType == "canonical") %>% subset(testisRestricted == "no")
# ORFs_no_seqs = as.character(ORFs_no_seqs$ORFpep)
# ## select ORFs - testisRestricted
# ORFs_yes_seqs = testisRestrictedGGTEx_Translated %>% subset(orfType == "canonical") %>% subset(testisRestricted == "yes")
# ORFs_yes_seqs = as.character(ORFs_yes_seqs$ORFpep)
# 
# ## compute log2ratio per letter - testisRestricted
# log2ratios_yes = calculate_log2ratio_df(ncORFs_yes_seqs, ORFs_yes_seqs)
