...
translated_in_2 = ribORF_humanBrain_Annotated_biotypes %>%
  select(orfID,gene_id,gene_type) %>%
  group_by(orfID,gene_id,gene_type) %>%
  count()
names(translated_in_2)[4] = "n"
translated_in_2 = translated_in_2 %>% 
  subset(n>1) %>% unique()

table(translated_in_2$gene_type)
ribORF_humanBrain_in2 = ribORF_humanBrain_Annotated_biotypes %>% subset(orfID %in% translated_in_2$orfID) 
write.csv(ribORF_humanBrain_in2, file.path(save_wd,"ribORF_humanBrain_translated_in_2.fulltable.csv"), row.names = F)

ribORF_humanBrain_in2 = ribORF_humanBrain_in2 %>% select(orfID, gene_id, gene_name, transcript_id, gene_type, orfType, length) %>% unique()
write.csv(ribORF_humanBrain_in2, file.path(save_wd,"ribORF_humanBrain_translated_in_2.simplified.csv"), row.names = F)

ORFs = as.data.frame(table(ribORF_humanBrain_in2$gene_type))
names(ORFs) = c("gene_type","num_ORFs")

## that correspond to:
ribORF_humanBrain_in2_genes = ribORF_humanBrain_in2 %>% select(gene_id, gene_type) %>% unique()
genes = as.data.frame(table(ribORF_humanBrain_in2_genes$gene_type))
...
num_ORFs_per_transcript = ribORF_humanBrain_in2 %>% 
...
ggplot(ribORF_humanBrain_in2, aes(x=gene_type, y=length, fill=gene_type)) +
  geom_boxplot() +
  scale_fill_manual(values=c("protein_coding"="#849DB1",
                         "lncRNA" = "#B66353",
                         "processed_pseudogene" = "#FBB04E",
...
wilcox.test(ribORF_humanBrain_in2$length ~ ribORF_humanBrain_in2$orfType)
```

```{r length_2, echo=F}
ribORF_humanBrain_in2$length_aa = ribORF_humanBrain_in2$length/3
ggplot(ribORF_humanBrain_in2, aes(x=length_aa, fill=gene_type, color="black")) +
...
ribORF_humanBrain_in2$start_codon = str_sub(ribORF_humanBrain_in2$orfID,-3)

ggplot(ribORF_humanBrain_in2, aes(x=gene_type, fill=start_codon)) +
...
# 
# ## select those that are in subset of interest --> ribORF_humanBrain_in2
# nrow(ribORF_humanBrain_in2)
# fastaCandidates_subset = fastaCandidates %>% subset(orfID %in% ribORF_humanBrain_in2$orfID)
# nrow(fastaCandidates_subset)
# 
# ## add the fasta sequences to the dataframe
# ribORF_humanBrain_in2_fastas = merge(fastaCandidates_subset, ribORF_humanBrain_in2, by="orfID")
# nrow(ribORF_humanBrain_in2_fastas)
# 
# ## translate the ORFs
# ribORF_humanBrain_in2_fastas$ORFpep = sapply(ribORF_humanBrain_in2_fastas$ORFseq, translate_dna_to_aa)
# write.csv(ribORF_humanBrain_in2_fastas, file.path(save_wd, "ribORF_humanBrain_in2.csv"), row.names = F)
...
## select ncORFs
ncORFs_seqs = ribORF_humanBrain_in2_fastas %>% subset(orfType == "noncoding")
ncORFs_seqs = as.character(ncORFs_seqs$ORFpep)

## ORFs
ORFs_seqs = ribORF_humanBrain_in2_fastas %>% subset(orfType == "canonical")
...
orf_TE_in2 = orf_TE %>% 
  ...
  unique() %>% 
  subset(orfID %in% ribORF_humanBrain_in2$orfID) %>% 
  unique()
nrow(orf_TE_in2)
...
brain_TPMs_with_ORFs = brain_TPMs %>% subset(transcript_id %in% orf_TE_in2$transcript_id)

#----------- sample 1 ---------#
## riboseq
orf_brain_1 = orf_TE %>% subset(Sample == "brain_1") %>% subset(orfID %in% orf_TE_in2$orfID) %>% select(orfID, ORF_type, transcript_id, f1Tpm)
...
#----------- sample 2 ---------#
## riboseq
orf_brain_2 = orf_TE %>% subset(Sample == "brain_2") %>% subset(orfID %in% orf_TE_in2$orfID) %>% select(orfID, ORF_type, transcript_id, f1Tpm)
...
#----------- sample 3 ---------#
## riboseq
orf_brain_3 = orf_TE %>% subset(Sample == "brain_3") %>% subset(orfID %in% orf_TE_in2$orfID) %>% select(orfID, ORF_type, transcript_id, f1Tpm)
